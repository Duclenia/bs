...
class LogActivity {
    ...
    {
        $date_format_type = ConfiguracaoGeral::findOrfail(1)->formato_data;
    ...
    {
        $date_formet = ConfiguracaoGeral::findOrfail(1)->formato_data;
    ...
    public static function CheckuserType() {
        ...
        $user = auth()->user();
        $data['id'] = ($user->user_type != 'Cliente') ? $user->admin->id : $user->id;
    ...
    public static function getNotifications()
    {
        ...
        $court_cases = Processo::where('next_date', '=', date('Y-m-d'))->where('is_nb', 'No')->count();
    ...
    {
        $row = Cliente::where('id', $id)->first();
        return $row->primeiro_nome . ' ' . $row->sobrenome;
    }
    ...
    public static function generateTasks() {
        ...
        $court_cases = Processo::where('next_date', '<', date('Y-m-d'))->where('is_nb', 'No')->get();
        ...
        if (count($court_cases) > 0) {
            foreach ($court_cases as $court_case) {
                ...
                $caseType = TipoProcesso::select('case_type_name')->where('id', $court_case->case_types)->first();
