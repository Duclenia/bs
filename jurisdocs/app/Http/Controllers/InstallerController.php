        ...
        try {
            ...
            if ($env_update) {
                ...
                if ($impSuccess) {
                    ...
                    $sqlPeS = "INSERT INTO pessoasingular (nome, sobrenome, sexo, pessoa_id)
                            VALUES ('" . $request->nome . "', '" . $request->sobrenome . "', '" . $request->sexo . "', '1')";

                    $sqlUser = "INSERT INTO utilizador(email, password, user_type)
                                VALUES ('" . $request->user_email . "', '" . bcrypt($request->user_pwd) . "', 'Admin')";

                    if (mysqli_query($conn, $sqlPeS)) {

                        if(mysqli_query($conn, $sqlUser)) {
                            
                            $sql = "INSERT INTO admin ( pessoasingular_id, utilizador_id, created_at)
                    VALUES ('5', '11','" . date('Y-m-d H:i:s') . "')";
                        ...
                        echo "Erro: " . $sqlPeS . "<br>" . mysqli_error($conn);
                    }
