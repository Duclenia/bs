    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="x_panel">
            <form id="add_client" name="add_client" role="form" method="POST" autocomplete="nope"
                  action="{{route('tarefas.update',encrypt($task->id))}}">
                {{ csrf_field() }}
                ...
                <div class="x_content">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12 form-group">
                            <label for="task_subject">{{__('Subject')}} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="task_subject" name="task_subject" value="{{old('task_subject', $task->task_subject ?? null) }}" autocomplete="off">
                        </div>

                        <div class="col-md-4 col-sm-12 col-xs-12 form-group">
                            <label for="start_date">Data de realiza&ccedil;&atilde;o <span class="text-danger">*</span></label>
                            <input type="text" readonly="" class="form-control dateFrom"
                                   id="start_date" name="start_date"

                                   value="{{ date($date_format_laravel, strtotime($task->inicio)) }}">
                        </div>

                        <div class="col-md-4 col-sm-12 col-xs-12 form-group">
                            <label for="hora_inicio">Hora de in&iacute;cio <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="hora_inicio" name="hora_inicio" required value="{{old('hora_inicio', $task->hora_inicio ?? null)}}">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12 form-group">
                            ...
                            <input type="text" name="hora_termino" class="form-control" id="hora_termino" value="{{old('hora_termino', $task->hora_termino ?? null)}}">
                        </div>
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12 form-group">
                            ...
                            <select class="form-control" id="priority" name="priority">
                                <option value="">Seleccionar prioridade</option>
                                @foreach(getTaskPriorityList() as $key=>$val)
                                <option value="{{$key}}"
                                        @if(isset($task) && $task->prioridade == $key) selected="" @endif
                                        >{{$val}}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-12 col-xs-12 form-group">
                            ...
                            <option value="other"
                                    @if(isset($task) && $task->rel_type=='other') selected="" @endif>Outro
                        </option>
                    </select>
                </div>
                ...
                <div class="col-md-4 col-sm-12 col-xs-12 form-group task_selection {{ $style }}">
                    <label for="fullname">Processo</label>
                    ...
                    <select  class="form-control selct2-width-100" id="related_id" name="related_id">
                        ...
                        <option value="{{$val->id}}"
                                ...
                                >
                    <strong>{{($val->tipo_cliente == '2') ?  $val->nome.' '.$val->sobrenome : $val->instituicao }}</strong><br>
                    <div>{{ 'No - '.$val->no_interno.'BSA' }}</div>
                    </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12 form-group">
                ...
                <textarea class="form-control" id="task_description"
                          name="task_description">{{old('task_description', $task->description  ?? null)}}</textarea>
            </div>
        </div>
